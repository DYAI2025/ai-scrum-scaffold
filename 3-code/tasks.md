...
| TASK-bff-checkout-endpoint | Implement `POST /api/checkout`: look up session, create Stripe Checkout Session, return `{ checkout_url }` | P1 | Done | [REQ-F-payment-integration](../1-spec/requirements/REQ-F-payment-integration.md) | TASK-bff-env-config, TASK-bff-session-store | 2026-04-15 | server/routes/checkout.ts; stripe SDK installed |
| TASK-bff-unlock-endpoint | Implement `GET /api/reading/unlock`: verify Stripe session, call FuFirE bootstrap (full), return `{ full_reading }` | P1 | Done | [REQ-F-reading-unlock](../1-spec/requirements/REQ-F-reading-unlock.md) | TASK-bff-checkout-endpoint, TASK-bff-session-store | 2026-04-15 | server/routes/unlock.ts; FuFirE client extracted to server/fufire.ts |
| TASK-bff-webhook-endpoint | Implement `POST /api/webhooks/stripe`: raw body middleware, signature validation, handle `checkout.session.completed` | P1 | Done | [REQ-SEC-data-protection](../1-spec/requirements/REQ-SEC-data-protection.md) | TASK-bff-env-config | 2026-04-15 | server/routes/webhook.ts; raw body before express.json() |
...
| TASK-spa-checkout-flow | Implement checkout: redirect to Stripe on CTA click, handle success redirect (`?session_id=`), call `/api/reading/unlock`, handle cancel/failure | P1 | Done | [REQ-F-payment-integration](../1-spec/requirements/REQ-F-payment-integration.md), [REQ-F-reading-unlock](../1-spec/requirements/REQ-F-reading-unlock.md) | TASK-bff-checkout-endpoint, TASK-bff-unlock-endpoint, TASK-spa-teaser-display | 2026-04-15 | Full checkout+unlock flow in App.tsx; RevealSection shows teaser vs full reading |
| TASK-spa-full-reading-display | Implement full reading display: Western, BaZi Four Pillars, Wu-Xing balance, soulprint sectors, harmony index | P1 | Done | [REQ-F-reading-unlock](../1-spec/requirements/REQ-F-reading-unlock.md) | TASK-spa-checkout-flow | 2026-04-15 | FullReadingPanel component; scrollable; partnership support |
...
| TASK-phase-3-manual-testing | Update runbook with complete character purchase flow test scenario (Stripe test mode) | P1 | Done | - | TASK-spa-full-reading-display | 2026-04-15 | Tests 9–13 added; Phase 3 env vars documented |
